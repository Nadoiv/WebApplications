...
class webSocket {
  ...
  broadcastDelete(doc) {
    this.wss.clients.forEach(client => {
      client.send(JSON.stringify({ action: 'carRemoved', data: doc }))
    })
  }

  broadcastAdd(doc) {
    this.wss.clients.forEach(client => {
      client.send(JSON.stringify({ action: "carAdded", data: doc }))
    })
  }

  broadcastUpdate(doc) {
    this.wss.clients.forEach(client => {
      client.send(JSON.stringify({ action: "carUpdated", data: doc }))
    })
  }
